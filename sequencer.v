...
//	03/08/02 Replaced header mux with barrel shifter
...
//	03/03/03 Fixed mpc response timeout, new barrel shifter for mpc response buffer number, deleted mpc machine
...
// CCB
	...
	input					clock_lock_lost;	// 40MHz main clock lost lock FF
...
// CLCT VME Configuration Ports
	...
	input	[MXL1A-1:0]		l1a_offset;			// L1A counter preset value
...
// Readout State Declarations
	...
	parameter xb07		=	12;	// Send B05 frame to begin scope
	parameter xmini		=	13;	// Send scope frames
	parameter xe07		=	14;	// Send E05 frame to end scope

	parameter xe0c		=	15;	// Send E0C marker to DMB
	parameter xmod40	=	16;	// Send 2 words to make word count multiple of 4
	parameter xmod41	=	17;	// Send 2 words to make word count multiple of 4

	parameter xe0f		=	18;	// Send E0F frame
	parameter xcrc0		=	19;	// Send crc frames
	parameter xcrc1		=	20;	// Send crc frames
	parameter xlast		=	21;	// Send last frame with word count

	parameter xpop		=	22;	// Pop data off stack, go back to idle
	parameter xhalt		=	23;	// Halted, wait for resume
...
// Bunch Crossing Counter, counts 0 to 3563, presets at resync or bxreset, stops counting, resumes at bx0
	...
	else if	(ttc_bx0   )	bxn_sync_err <= !bxn_sync || bxn_sync_err || clock_lock_lost;
	else if (bxn_sync  )	bxn_sync_err <= !ttc_bx0  || bxn_sync_err || clock_lock_lost;
	end

	wire  sync_err_cnt_en    = (ttc_bx0 && !bxn_sync) || (bxn_sync && !ttc_bx0);
	assign clct_bx0_sync_err = sync_err_cnt_en;					// TMB clock pulse count err bxn!=0+offset at ttc_bx0 arrival

// Shadow bunch crossing counter counts 0 to 3563 for L1A because it has a separate preset
	reg	[MXBXN-1:0]	bxn_counter_l1a;
	...
	else					bxn_counter_l1a <= bxn_counter_l1a+1;
	end

// Orbit Counter, counts bx0s from bx counter
	reg [MXORBIT-1:0] orbit_counter;
	...
	else if (orbit_cnt_en   ) orbit_counter=orbit_counter+1;
	end
...
// Pre-trigger counter, presets at evcntres or resync
	reg	 [MXCNTVME-1:0]	pretrig_counter;
	...
	else if (pretrig_cnt_en   ) pretrig_counter = pretrig_counter+1;
	end

// CLCT counter, presets at evcntres or resync
	reg	 [MXCNTVME-1:0]	clct_counter;
	...
	else if (clct_cnt_en   ) clct_counter = clct_counter+1;
	end

// ALCT counter, presets at evcntres or resync
	reg	 [MXCNTVME-1:0]	alct_counter;
...
// Trigger counter, presets at evcntres or resync, counts all triggers including ones not sent to mpc
	...
	else if (trig_cnt_en   ) trig_counter = trig_counter+1;
	end

// Level Accept Rx Counter, counts ccb_l1accepts received, presets at evcntres or resync
	reg	 [MXL1ARX-1:0]	l1a_rx_counter;			// L1As received from ccb
	...
	else if (l1a_rxcnt_en ) l1a_rx_counter = l1a_rx_counter+1;
	end

	assign l1a_rx_counter_plus1 = l1a_rx_counter+1;

// Readout counter, counts actual L1A readouts
	reg	[MXL1ARX-1:0]	readout_counter=0;		// Readout counter
	reg	 [NHBITS-1:0]	header_cnt;				// Header  counter

	wire rocnt_en = (read_sm==xckstack) && !buf_q_empty && !no_daq;

	always @(posedge clock) begin
	if 		(l1a_cnt_reset) readout_counter = 0;
	else if (rocnt_en     ) readout_counter = readout_counter+1;
	end
...
// On CCB or VME command, start CFEB pattern injectors
	reg	[MXCFEB-1:0]	injector_go_cfeb;
...
// Throttle state timer, reduce trigger rate
	reg	 [MXTHROTTLE-1:0] throttle_cnt;
...
// Record which CFEBs were hit at pretrigger
	reg [MXCFEB-1:0] cfeb_hit_at_pretrig;
...
// Delay TMB active feb list 1bx so it can share tmb+1bx RAM
	reg [MXCFEB-1:0] tmb_aff_list_ff;
...
// Latch CLCTs for VME
	...
	always @(posedge clock) begin
	if (clct_pretrig) begin		// Clear clcts in case event gets flushed
	clct0_vme		<=	0;
	clct1_vme		<=	0;
	clctc_vme		<=	0;
	clctf_vme		<=	0;
...
// Counter registers
	...
	reg [MXCNT:MNCNT]	cnt_en;							// Counter increment enables
...
// L1A Barrel shifter write address increments every cycle, read address is later in time by l1a_delay
	reg [7:0]	l1a_delay_wadr=0;	
	reg [7:0]	l1a_delay_radr=8'hFF;

	always @(posedge clock) begin
	l1a_delay_wadr = l1a_delay_wadr + 1;
...
// Push TMB event token into a 16-stage FF shift register for L1A matching
	...
	always @(posedge clock) begin
	l1a_sr[15:0] <= {l1a_sr[14:0],tmb_push_dly};
	end
...
// Decode L1A window width setting to select which l1a_sr stages to include in l1a_window
...
// L1A window matching shift registers
	...
	while (i<=14) begin
	if (l1a_match) begin				// On L1A load tag, window position, and L1A counter into shift register 
	if (l1a_sr[i] && l1a_sr_include[i] && !l1a_tag_sr[i]) begin
...
// Extract pushed counters and match results from sr stage after window closed
	assign tmb_push_sr	 = l1a_sr[winclosed];				// TMB token emerges from last window position 1bx before tag
...
// L1A window width is generated by a pulse propagating down the enabled l1a_sr stages	
	assign l1a_window_open    = |(l1a_sr & l1a_sr_include);
	assign l1a_window_haslcts = |(l1a_sr & l1a_sr_include & ~l1a_tag_sr);
...
// Push event address into fence queue to protect raw hits and queue for readout to DMB
	reg [MXBADR-1:0]	buf_push_adr;						// Address of write buffer to allocate	
	reg [MXBDATA-1:0]	buf_push_data;						// Data associated with push_adr
...
// Header word counter
	...
	else if (read_sm == xheader) header_cnt = header_cnt + 1;	// sync count
	end
...
// Full Header-mode words 8-to-EOB: Event Counters
	...
	assign	header08_[14]		=	clock_lock_lost;			// Main DLL lost lock
...
// ALCT Trigger Data
	...
	assign	header30_[11:7]		=	cfeb_badbits_found[4:0];	// CFEB[n] has at least 1 bad bit							// 6 Free
...
// Buffer Status
	...
	assign	header38_[14]		=	buf_stalled;				// Buffer write pointer hit a fence and stalled
...
// Store header in barrel shifter
...
// Miniscope FIFO RAM write data
	assign fifo_wdata_mini[0]		= any_cfeb_hit;				// Any CFEB over threshold
	assign fifo_wdata_mini[3:1]		= clct_sm_vec[2:0];			// Pre-trigger state machine
	assign fifo_wdata_mini[6:4]		= hs_hit_1st[2:0];			// CLCT0 number hits after drift
	assign fifo_wdata_mini[9:7]		= hs_hit_2nd[2:0];			// CLCT1 number hits after drift
	assign fifo_wdata_mini[10]		= alct0_vpf_tprt;			// ALCT vpf in TMB after pipe delay, unbuffered real time
	assign fifo_wdata_mini[11]		= alct1_vpf_tprt;			// ALCT vpf in TMB after pipe delay, unbuffered real time
	assign fifo_wdata_mini[12]		= clct0_vpf;				// CLCT vpf in TMB
	assign fifo_wdata_mini[13]		= clct1_vpf;				// CLCT vpf in TMB
	assign fifo_wdata_mini[14]		= clct_window_tprt;			// CLCT matching window in TMB
	assign fifo_wdata_mini[15]		= 0;						// No readout for this bit, taken by ddu special
...
// Miniscope Readout
	...
	assign rd_mini_offset = 0;											// RAM address rd_fifo_adr offset for rpc read out
...
// Output calibration signals to help tune external L1A arrival time on CCB front panel
	...
	stat_l1a			<=	l1a_pulse;
	stat_dmb			<=	(read_sm == xheader) || 
							(read_sm == xe0b   ) || (read_sm == xdump ) || (read_sm == xe0c  ) ||
							(read_sm == xb04   ) || (read_sm == xrpc  ) || (read_sm == xe04  ) ||
							(read_sm == xb05   ) || (read_sm == xscope) || (read_sm == xe05  ) ||
							(read_sm == xmod40 ) || (read_sm == xmod41) ||
						    (read_sm == xcrc0  ) || (read_sm == xcrc1 ) ||
							(read_sm == xe0f   ) || (read_sm == xlast );
...
// Condense state machine vectors into binary encoding for diagnostic status readout	
	...
	xe05:		read_sm_vec <= 11;
	xe0c:		read_sm_vec <= 12;
	xmod40:		read_sm_vec <= 13;
	xmod41:		read_sm_vec <= 14;
	xe0f:		read_sm_vec <= 15;
	xcrc0:		read_sm_vec <= 16;
	xcrc1:		read_sm_vec <= 17;
	xlast:		read_sm_vec <= 18;
	xpop:		read_sm_vec <= 19;
	xhalt:		read_sm_vec <= 20;
	default		read_sm_vec <= 21;
	endcase
	end

// Sequencer Status for VME
	reg [11:0] sequencer_state;
...
// DMB write address counter for VME readback
	...
	else if (wadr_cnten) wadr = wadr + 1;
	end

// Buffer DMB write address counter and data
	reg [MXRAMADR-1:0]	dmb_wdcnt;
 	reg	[MXRAMADR-1:0]	seq_wadr;
	reg					seq_wr;
...
// State machine declarations
	...
	reg [1:0] cb_cnt;
	always @(posedge clock) begin
	if (cb_sm==cb_clear) cb_cnt=cb_cnt+1;
	else				 cb_cnt=0;
	end
